...
```{r}
nematode_meancv <- data.frame(variable = rep(NA, 26), #26 because that's how many variables we'll calculate mean and cv for
                              mean = rep(NA, 26),
                              cv = rep(NA, 26))
...
```{r}
for (i in 3:ncol(nematode_w)) { # why 3 here? 
  ...
  nematode_meancv$variable[[i-2]] <- varname # why i -2?
  nematode_meancv$mean[[i-2]] <- round(statsum$mean,3)
  nematode_meancv$cv[[i-2]] <- round(statsum$cv,0)
